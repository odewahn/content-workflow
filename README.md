...
* The *WYSIWYG* model combines the editor and the final format into one seamless environment. For example: Jupyter Notebooks, Nteract, Zepplin, Toree, etc.  The strength of the model is that it is generally simple for authors and other people in the workflow to use (assuming they have the right software installed).  The weakness is that you're limited to whatever tools that system supports, but the huge interest in the Jupyter is largely due to the vast array of content types.
